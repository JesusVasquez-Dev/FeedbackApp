...
export async function listTeam(userId: string, limit = 6, email?: string): Promise<TeamMember[]> {
  const qs = `limit=${encodeURIComponent(String(limit))}` + (email ? `&email=${encodeURIComponent(email)}` : '');
  const res = await api<{ team: TeamMember[] }>(`/me/${encodeURIComponent(userId)}/team?${qs}`);
  return res.team || [];
}
...
export async function getOnboardingStatus(userId: string, email?: string): Promise<OnboardingStatus> {
  const qs = email ? `?email=${encodeURIComponent(email)}` : '';
  return api<OnboardingStatus>(`/me/${encodeURIComponent(userId)}/status${qs}`);
}
