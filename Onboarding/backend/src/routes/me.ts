...
async function selectSingleWithFallback(table: string, column: string, value: any) {
  ...
  if (
    q.error?.message?.includes('permission') ||
    q.error?.message?.includes('relation') ||
    q.error ||
    !q.data // also fallback when no row found in primary schema
  ) {
...
async function listByCompanyWithFallback(companyColumn: string, companyId: any, excludeUserId: string, excludeEmail?: string) {
  let qb = supabase
    .schema(SCHEMA)
    .from('profiles')
    .select('id, user_id, email, full_name, first_name, last_name, job_title, location, timezone, bio')
    .eq(companyColumn, companyId)
    .limit(64);
  // IMPORTANT: do not filter with `user_id != <id>` because rows with NULL user_id will be excluded by SQL.
  // Prefer excluding the requester by email (more reliable across legacy rows).
  if (excludeEmail) qb = qb.neq('email', excludeEmail);
  else qb = qb.neq('user_id', excludeUserId);
  const q = await qb;
  if (q.error) {
    let altQb = supabase
      .schema('public')
      ...
      .eq(companyColumn, companyId)
      .limit(64);
    if (excludeEmail) altQb = altQb.neq('email', excludeEmail);
    else altQb = altQb.neq('user_id', excludeUserId);
    const alt = await altQb;
    return alt;
  }
  return q;
}
...
meRouter.get('/:userId/status', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Single source of truth: the profile row tied to the logged-in user (profiles.user_id).
    // If a legacy profile exists by email but isn't linked to user_id yet, merge it by setting user_id.
    let profileRow = p;
    if (!profileRow.data && email) {
      const byEmail = await selectSingleIlikeWithFallback('profiles', 'email', email);
      if (byEmail.data) {
        const existingUserId = (byEmail.data as any)?.user_id;
        if (!existingUserId || String(existingUserId) !== String(userId)) {
          const merged = await updateWhereWithFallback('profiles', 'email', email, { user_id: userId });
          profileRow = merged.data ? merged : byEmail;
        } else {
          profileRow = byEmail;
        }
      }
    ...
    const rawCP: any = (profileRow.data as any)?.CompletionPercent;
    const completionPercent: number | null =
      typeof rawCP === 'number' ? rawCP : (typeof rawCP === 'string' && rawCP.trim() !== '' && !isNaN(Number(rawCP)) ? Number(rawCP) : null);
    let completed: boolean;
    if (completionPercent != null) {
      completed = completionPercent >= 100;
    } else {
      // Fallback legacy rule: all three exist
      const profileOk = !!profileRow.data;
      const preferencesOk = !!pref.data;
      const surveyOk = !!s.data;
      completed = profileOk && preferencesOk && surveyOk;
    }
    return res.json({ completed, completionPercent: completionPercent ?? null, sections: { profile: !!profileRow.data, preferences: !!pref.data, survey: !!s.data } });
  } catch (e: any) {
...
meRouter.get('/:userId/team', async (req: Request, res: Response) => {
  ...
  try {
    // Load requester's profile to determine company id (prefer user_id; fallback to email)
    let myProfile = await selectSingleWithFallback('profiles', 'user_id', userId);
    if ((!myProfile.data || myProfile.error) && email) {
      myProfile = await selectSingleIlikeWithFallback('profiles', 'email', email);
    }
    if (myProfile.error) return res.status(500).json({ error: myProfile.error.message });

    const p = (myProfile.data || {}) as any;
    const companyId = p.companyID ?? p.CompanyID ?? p.CompanyId ?? p.company_id ?? p.companyId ?? null;
    if (companyId == null) {
      return res.json({ team: [] });
    }

    // Fetch profiles and filter by companyId in JS to avoid column casing/type mismatch issues.
    // This intentionally includes rows with NULL user_id (employees who haven't logged in yet).
    // Use select('*') to avoid failing when a column name differs across environments.
    ...
    if (q.error) {
      const alt = await supabase
        ...
        .select('*')
        .limit(256);
      // If public schema doesn't have the table, keep the original error.
      if (alt.error) return res.status(500).json({ error: q.error.message });
      q = alt as any;
    }

    const norm = (v: any) => (v == null ? '' : String(v).trim());
    const myCompany = norm(companyId);
    const rowsAll = Array.isArray(q.data) ? q.data : [];
    const rows = rowsAll
      .filter((r: any) => {
        const c = r.companyID ?? r.CompanyID ?? r.CompanyId ?? r.company_id ?? r.companyId ?? null;
        return norm(c) !== '' && norm(c) === myCompany;
      })
      .filter((r: any) => {
        if (email && r.email && norm(r.email).toLowerCase() === norm(email).toLowerCase()) return false;
        if (r.user_id && String(r.user_id) === String(userId)) return false;
        return true;
      })
      .map((r: any) => ({
        id: r.id,
