...
export const SelectContent: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className = "", ...props }) => {
  ...
  return (
    <div className={`absolute z-50 mt-1 w-full rounded-md border bg-white text-foreground shadow-lg max-h-60 overflow-auto ${className}`} {...props}>
      {props.children}
    </div>
  );
};

export const SelectItem: React.FC<{ value: string; label?: string; children: React.ReactNode; className?: string }>
  = ({ value, label, children, className = "" }) => {
  const ctx = useContext(SelectCtx)!;
  return (
    <div
      role="option"
      onClick={() => ctx.setValue(value, label ?? (typeof children === "string" ? children : undefined))}
