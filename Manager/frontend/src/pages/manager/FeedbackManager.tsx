...
export default function FeedbackManager() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Load feedback for those team members
        const teamIds = teamRows.map((t) => t.user_id).filter(Boolean);
        const fb = await supabase
          .schema(SCHEMA)
          .from("EmployeeFeedback")
          .select("id, created_at, category, OverallRating, YourFeedback, idProfile, Status")
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      if (filterUser && r.idProfile !== filterUser) return false;
  ...
  }, [rows, filterUser, statusFilter, categoryFilter, filterFromDate, filterToDate]);
      ...
      <div className="space-y-3">
        <div className="grid gap-3 md:grid-cols-3">
          <div>
            ...
            <select className="mt-1 w-full border rounded-md px-3 py-2" value={filterUser} onChange={(e) => setFilterUser(e.target.value)}>
          ...
          <div>
            ...
            <select className="mt-1 w-full border rounded-md px-3 py-2" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
              <option value="">All</option>
          ...
          <div>
            ...
            <select className="mt-1 w-full border rounded-md px-3 py-2" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}>
              <option value="">All</option>
          ...
          <div key={r.id} className="emp-card p-3">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="font-medium">{teamMap.get(r.idProfile) || r.idProfile}</div>
                <div className="text-sm text-muted-foreground">
                  {r.created_at ? new Date(r.created_at).toLocaleString() : ""} • {r.category || "General"}
                </div>
                {r.YourFeedback && (
                  <div className="mt-2 whitespace-pre-wrap text-sm">{r.YourFeedback}</div>
                )}
              </div>
