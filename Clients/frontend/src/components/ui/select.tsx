...
export const Select: React.FC<{ value?: string; onValueChange?: (v: string) => void; className?: string; children: React.ReactNode }>
  = ({ value: controlled, onValueChange, className = "", children }) => {
...
export const SelectContent: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ className = "", ...props }) => {
  ...
  return (
    <div
      className={`absolute z-50 mt-1 min-w-full rounded-md border border-gray-200 bg-white text-gray-900 shadow-lg ${className}`}
      {...props}
    />
  );
};
