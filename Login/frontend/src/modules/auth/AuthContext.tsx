...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const sub = supabase.auth.onAuthStateChange((_event: string, sess: Session | null) => {
        setSession(sess);
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    async signOut() {
      try {
        // Global sign out invalidates refresh tokens to ensure session is closed everywhere
        await supabase.auth.signOut({ scope: 'global' });
      } finally {
        // Proactively clear local state immediately
        setSession(null);
        setUser(null);
      }
    },
